function checkValidation() {
  ...
  for (var i = 0; i < validations.length; i++) {
    if (validations[i][0] == null) {
      value = false;
      input = ui.alert(mainSheet.getRange(i + 2, 1).getValue(), 'No checkbox was found in this team. Should this team be included in the report?', ui.ButtonSet.YES_NO);
  ...
  for (var i = 0; i < validations.length; i++) {
    if (validations[i][0] == null) {
      value = false;
      input = ui.alert(indvSheet.getRange(i + 2, 1).getValue(), 'No checkbox was found for this CA. Should this CA be included in the report?', ui.ButtonSet.YES_NO);
      if (input == ui.Button.YES) { value = true; }
      else if (input == ui.Button.CLOSE) { return false;  }
